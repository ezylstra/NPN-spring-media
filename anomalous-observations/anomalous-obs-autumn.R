# Anomalous observations script, for autumn phenology
# 29 Sep 2025
...
# Because we're interested in autumn phenology, will probably use individual
# phenometrics based on a summer year (Jul - Jun). Will need:
  # Individual phenometrics for current year
  # Individual phenometrics for prior years (for current year species)
...
agdd_breaks <- c(-4000, -200, 200, 4000)
...
elev_fill <- data.frame(elev_fill) %>%
  ...
  select(site_id, elev_new)
prior <- prior %>%
  left_join(elev, by = "site_id") %>%
  rename(elev = elev_new)

# Save to file:
# write.csv(current, 
#           here("anomalous-observations", "current_autumn_20250929.csv"),
#           row.names = FALSE)
# write.csv(prior,
#           here("anomalous-observations", "prior_autumn_20250929.csv"),
#           row.names = FALSE)

# Compare current year observations with distribution of first observation dates
# for plants within xx km and xx m elevation ----------------------------------#

current <- read.csv(here("anomalous-observations", "current_autumn_20250929.csv"))
prior <- read.csv(here("anomalous-observations", "prior_autumn_20250929.csv"))
...
# Loop through phenophase and create ggplot objects with histograms
...
eo_table <- quants_r_eo %>%
  ...
  select(phenophase_id, phenophase, common_name, func_type, id, state, lat, lon, 
...
# Tables with number early/outlier by state, by species..... ################
